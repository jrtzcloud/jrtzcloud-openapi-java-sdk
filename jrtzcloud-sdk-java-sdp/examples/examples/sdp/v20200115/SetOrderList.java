...
public class SetOrderList {
    ...
    public static final String PATH = "/sdp-cust/externalCall/setOrderList";
...
//            httpProfile.setReqMethod("GET"); // get请求(默认为post请求)
            ...
            SetOrderListRequest req = new SetOrderListRequest();

            // 填充请求参数,这里request对象的成员变量即对应接口的入参
            req.setAppId("001");//券商id,今天投资系统分配
            req.setAccountId("1001");//券商系统内部C用户唯一Id
            req.setGoodId("CU0001");//券商产品Id
            req.setGoodName("个股诊断");//券商产品名称
            req.setOrderTime("2020-01-16 00:00:00");//订购时间
            req.setStartTime("2020-01-16 00:00:00");//服务开始时间
            req.setEndTime("2020-02-15 00:00:00");//服务结束时间
            req.setOrderNum("XH000001");//渠道订购订单号
            req.setAmount(30000L);//订单金额，单位分
            req.setNumbers(6);//计量数量，举例：6个月
            req.setUnit(3);//计量单位 0 为定义 1 天 2 周 3 月 4 季度 5 年 6 次
            req.setOrderType(1);//订购状态信息 1 支付 2 退款
            ...
            SetOrderListResponse resp = client.SetOrderList(req);

            System.out.println("\n");

            // 输出json格式的字符串回包
            System.out.println(SetOrderListResponse.toJsonString(resp));
