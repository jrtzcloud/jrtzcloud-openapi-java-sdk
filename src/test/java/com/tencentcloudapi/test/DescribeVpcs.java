package com.tencentcloudapi.test;
...
public class DescribeVpcs {
    public static void main(String[] args) {
        try {
            ...
            Credential cred = new Credential("AKIDRHUO70VGLbgbMxRbGlRxXSC3tZKxshpn", "117bvELH2iHVM3Zu56zzbpeIm5DWM5b2");

            // 实例化一个http选项，可选的，没有特殊需求可以跳过
            HttpProfile httpProfile = new HttpProfile();
//            httpProfile.setReqMethod("GET"); // get请求(默认为post请求)
            httpProfile.setConnTimeout(30); // 请求连接超时时间，单位为秒(默认60秒)
            httpProfile.setEndpoint("vpc.ap-shanghai-fsi.tencentcloudapi.com"); // 指定接入地域域名(默认就近接入)
            ...
            // 实例化要请求产品(以vpc为例)的client对象,clientProfile是可选的
            VpcClient client = new VpcClient(cred, "ap-shanghai", clientProfile);

            // 实例化一个vpc实例信息查询请求对象,每个接口都会对应一个request对象。
...
//            req.setFilters(new Filter[] { respFilter }); // Filters 是成员为Filter对象的列表
            ...
            req = DescribeVpcsRequest.fromJsonString(params, DescribeVpcsRequest.class);
            ...
            // 输出json格式的字符串回包
            System.out.println(DescribeVpcsResponse.toJsonString(resp));
