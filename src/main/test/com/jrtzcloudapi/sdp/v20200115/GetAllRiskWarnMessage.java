...
public class GetAllRiskWarnMessage {
    ...
    public static final String PATH = "/sdp-cust/riskWarn/getAll";
...
//            httpProfile.setReqMethod("GET"); // get请求(默认为post请求)
            ...
            httpProfile.setEndpoint("kong.test.investoday.net"); // 指定接入地域域名(默认就近接入)
            ...
            GetAllRiskWarnMessageRequest req = new GetAllRiskWarnMessageRequest();

            // 填充请求参数,这里request对象的成员变量即对应接口的入参
            req.setPubDt("2020-01-01");
            ...
            GetAllRiskWarnMessageResponse resp = client.GetAllRiskWarnMessage(req);

            System.out.println("\n");

            // 输出json格式的字符串回包
            System.out.println(GetAllRiskWarnMessageResponse.toJsonString(resp));
